...
static const rclcpp::Logger LOGGER = rclcpp::get_logger("left_mtc_node");
namespace mtc = moveit::task_constructor;

class LeftMTCTaskNode
{
public:
  explicit LeftMTCTaskNode(const rclcpp::NodeOptions& options);
...
rclcpp::node_interfaces::NodeBaseInterface::SharedPtr LeftMTCTaskNode::getNodeBaseInterface()
{
  return node_->get_node_base_interface();
}

LeftMTCTaskNode::LeftMTCTaskNode(const rclcpp::NodeOptions& options)
  : node_{ std::make_shared<rclcpp::Node>("left_mtc_node", options) }
{
}

mtc::Task LeftMTCTaskNode::createTask()
{
  mtc::Task task;
  task.stages()->setName("left_arm_task");
  task.loadRobotModel(node_);

  // Thông số Tay Trái từ code gốc
  ...
  // 3. Chuỗi Pick
  {
    ...
    // 3.1 Bỏ qua va chạm (SỬA LỖI TỰ VA CHẠM TAY KẸP TẠI ĐÂY)
    {
      ...
      allow_coll->allowCollisions(hand_links, hand_links, true); // Cho phép các link tay kẹp tự chạm nhau
    ...
    // 3.3 Tạo Pose gắp + IK
    {
      ...
      ik_wrapper->setIgnoreCollisions(true); // Bỏ qua xung đột mesh nhỏ khi giải IK
  ...
  // 6. Trở về vị trí ban đầu
  {
    ...
    home->setGoal("left_test_configuration");
...
void LeftMTCTaskNode::doTask()
{
  task_ = createTask();
  try {
    task_.init();
  } catch (mtc::InitStageException& e) {
    RCLCPP_ERROR_STREAM(LOGGER, "========= LỖI INIT MTC STAGE (LEFT) =========\n" << e);
    return;
  }

  if (!task_.plan(5)) {
    RCLCPP_ERROR_STREAM(LOGGER, "Left Task planning failed");
    return;
  }
  ...
  if (result.val != moveit_msgs::msg::MoveItErrorCodes::SUCCESS) {
    RCLCPP_ERROR_STREAM(LOGGER, "Left Task execution failed");
    return;
  }
  RCLCPP_INFO(LOGGER, "Left Task executed successfully");
}
...
int main(int argc, char** argv)
{
  ...
  auto mtc_task_node = std::make_shared<LeftMTCTaskNode>(options);
